...
MENUS = [
    ...
    [("Explore Solutions", [("Who It’s For", "solutions/"), ("Personal", "solutions/#personal"),
                            ...
                            ("Business and Government", "solutions/#institutions")]),
     ("Education", [("Oplo Edu", "edu/"), ("OEdu", OEDU), ("Who It’s For", "edu/#who")]),
...
'''
  ...
  <div class="well">
    ...
    <p class="cta-row reveal d2">
      <a class="cta" href="learn/">See how OEdu works</a>
      <a class="cta" href="../contact/">Talk to us</a>
    </p>
  </div>
</section>
...
# ==========================================================================
# OEdu. The online-school product page.
#
...
def learn_page():
    ...
    out = head(depth, "OEdu — online school",
...
'''
    notes = [
        "OEdu is in development. The console shown is a wireframe of the interface being designed, "
    ...
    <div class="people reveal d1">
      <div class="person">
        <span class="shot">
          ...
          <img src="../../assets/img/saswat-ji.jpg" alt="Saswat Ji" width="560" height="560"
...
# ------------------------------------------------------------ OEdu welcome
# edu.oplocloud.com is not this site, but the page people meet there before
# signing in wears this site's bar, chapter bar and footer: the same markup,
# built from the same NAV and FOOTER by the same functions, so the two cannot
# drift. It is not a whole page — the app owns learn/index.html — so only the
# regions between <!-- oplo:name --> and <!-- /oplo:name --> are replaced.
#
# Links are made absolute, because a relative one would resolve against
# edu.oplocloud.com. "Sign in" is OEdu's own sign-in, not this site's.
#
# The CSS is this site's own rules too, cut from oplo-design.css by section
# heading and scoped under .ld, so none of it can reach the app around it.
SITE = "https://oplocloud.com/"
OEDU_PAGE = "learn/index.html"
OEDU_CSS = "learn/oplo-chrome.css"
...
def absolute(html):
    ...
    def fix(m):
        ...
        if url.startswith(("http", "mailto:", "#", "data:")):
            return m.group(0)
        return 'href="' + SITE + ("" if url == "index.html" else url) + '"'
    return re.sub(r'href="([^"]*)"', fix, html)


def css_section(src, title):
    """One headed section of oplo-design.css, up to the next heading."""
    start = re.search(r"/\* -+ " + re.escape(title) + r"\b", src)
    if not start:
        raise SystemExit(f"oplo-design.css has no section called {title!r}")
    end = re.search(r"\n/\* [-=]{3,}", src[start.end():])
    return src[start.start(): start.end() + (end.start() if end else len(src))]
...
def scope_css(css, prefix):
    """Every selector in `css` prefixed, @-rules left as they are. The input
    is this site's own stylesheet, with no strings holding braces."""
    ...
    for ch in css:
        if ch == "{":
            ...
            if head.startswith("@"):
                ...
                out.append(", ".join(prefix + " " + s for s in split_top(head)) + " {")
            buf = ""
        elif ch == "}":
            body = " ".join(buf.split())
            if body:
...
def oedu_chrome():
    ...
    # source (the font stacks do), so each is folded onto one before it is
