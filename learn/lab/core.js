...
window.OPLO_LAB = (function () {
  ...
  function unitSeq(u) {
    ...
    u.lessons.forEach(function (l) {
      var item = { kind: "lesson", k: l.k, title: l.title, label: "Lesson " + l.k, done: lessonDone(l) };
      item.children = l.steps.map(function (s, i) {
        return { kind: "step", k: i + 1, title: s.prompt || "Step " + (i + 1), label: l.k + "." + (i + 1), done: false };
      });
      seq.push(item);
      u.quizzes.filter(function (q) { return q.after === l.k; }).forEach(function (q) {
  ...
  function frame(host, ctx, u, here) {
    ...
    function section(key, name, items) {
      ...
      items.forEach(function (it) {
        ...
        if (it.children) {
          b.innerHTML += svg(ICON.down);
          var subUl = el("ul", "lb-subrows folded");
          it.children.forEach(function (child) {
            var cli = el("li");
            var cb = button("lb-row k-step",
              '<span class="lb-row-ico">' + svg(ICON.doc, false) + "</span>" +
              '<span class="lb-row-t">' + '<i>' + child.k + "</i>' + esc(stripMath(child.title)) + "</span>"
            );
