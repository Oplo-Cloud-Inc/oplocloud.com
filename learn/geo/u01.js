/* ==========================================================================
   ...
   Built for a student working alone, so every idea is found, then shown,
   then done, then said back — never just told. A lesson goes

     find it     a point to drag while its image follows (the map widget),
                 with the numbers of both in a table, until the rule is there
                 to be read off — and then a question asking for it
     watch it    a worked example a line at a time, its picture building up
                 beside the lines the way a teacher draws on the board
     do it       the same thing with new numbers, hints that walk the steps
                 (the last one draws the answer), and figures drawn by hand
                 on the grid (the plot widget), corner by corner
     a step on   the same idea with a twist, and the slip people make with it
     say it      now and then, an explanation in the student's own words,
                 set beside a model answer to rate honestly
     recap       the lesson on one card, to come back to
   ...
   blue is where it ends up (the image), green is a target, orange is a move
   (an arrow, a turn, a centre), purple is a mirror line.

   Nine lessons, in the order of the unit: the words of geometry (two
   ...
   and a review — every rule on one card, then a mixed set drawn from the
   skills. Eighteen skills practise it, one for each exercise in the unit;
   the shape skills are drawn, not matched. Three quizzes check it, and the
   unit test draws from every skill. Standards: HSG.CO.A.1–5, HSG.SRT.A.1.
   ========================================================================== */
(function () {
  "use strict";
  ...
  /* A rule found by trying it: drag a point and watch where it goes (the map
     widget), with the numbers of both in a table — then say what the table
     shows. */
  function explore(map, o) {
    return { type: "learn", kicker: o.kicker || "Explore", prompt: o.prompt, then: o.then, gate: true,
      scene: { type: "map", map: map, start: o.start || [3, 1], need: o.need || 4, x: o.x || [-6, 6], y: o.y || [-6, 6], u: o.u || 26, show: o.show } };
  }
  /* Draw the image: click where each corner goes (the plot widget), the way
     it is done on paper. */
  function draw(o) {
    return { type: "plot", prompt: o.prompt, skill: o.skill, show: o.show, target: o.target, names: o.names,
             x: o.x || [-6, 6], y: o.y || [-6, 6], u: o.u || 26, hints: o.hints, why: o.why, kicker: o.kicker };
  }
  /* A lesson's last card: everything it gave, on one card to come back to. */
  ...
  /* A small picture for a worked answer. */
  function shown(x, y, items, alt) { return grid(x, y, items, { u: 18, w: 250, alt: alt || "The worked answer on a grid." }); }
...
var SKILLS = [
      ...
      gen: function (R) {
        ...
        return { type: "pair", prompt: ask, answer: [x + a, y + b], near: near,
          ...
          why: "$" + pt(x, y) + " \\to " + pt(x + a, y + b) + "$: " + across(a) + ", " + upDown(b) + ". " +
            shown([-9, 9], [-9, 9], [{ steps: [[x, y], [x + a, y + b]] }, { pt: [x, y], name: "P" }, { pt: [x + a, y + b], name: "P'", c: "blue" }]) };
      } },
    ...
    { id: "geo-tshape", title: "Translating shapes", lesson: 4,
      gen: function (R) {
        ...
        return { type: "plot", prompt: "Draw the image of $\\triangle ABC$ under the translation $(x, y) \\to (x " + signed(a) + ", y " + signed(b) + ")$.",
          show: [pre(sh)], target: t, x: [-8, 8], y: [-7, 7], u: 22,
          hints: ["One corner at a time: each one moves " + across(a) + " and " + upDown(b) + ". Start with $A" + P(sh[0]) + "$.",
                  "$A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$."],
          why: "Every corner moves " + across(a) + " and " + upDown(b) + ": $A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$. " +
            shown([-8, 8], [-7, 7], sh.map(function (p, i) { return { arrow: [p, t[i]], dash: true }; }).concat([pre(sh), img(t)])) };
      } },
    { id: "geo-tchal", title: "Translation challenge", lesson: 4,
      gen: function (R) {
        var x = R.int(-6, 6), y = R.int(-6, 6), a = R.nz(-5, 5), b = R.nz(-5, 5), u = R.int(-6, 6), v = R.int(-6, 6);
        while (u === x && v === y) u = R.int(-6, 6);
        var ans = [u + a, v + b];
        return { type: "pair", prompt: "A translation takes $" + pt(x, y) + "$ to $" + pt(x + a, y + b) + "$. Where does the **same** translation take $" + pt(u, v) + "$?",
          answer: ans,
          near: [{ v: [x + a, y + b], fb: "That's where the first point went. Find the move first, then do the same move to $" + pt(u, v) + "$." },
                 { v: [u - a, v - b], fb: "That's the move done backwards. New minus old: " + across(a) + " and " + upDown(b) + "." }]
            .filter(function (n) { return n.v[0] !== ans[0] || n.v[1] !== ans[1]; }),
          hints: ["First find the move — new minus old: $" + (x + a) + " - (" + x + ")$ across, and $" + (y + b) + " - (" + y + ")$ up.",
                  "It's " + across(a) + " and " + upDown(b) + ": $(x, y) \\to (x " + signed(a) + ", y " + signed(b) + ")$.",
                  "Now do that to $" + pt(u, v) + "$: $(" + u + " " + signed(a) + ",\\; " + v + " " + signed(b) + ")$."],
          why: "The translation is $(x, y) \\to (x " + signed(a) + ", y " + signed(b) + ")$, so $" + pt(u, v) + " \\to " + P(ans) + "$." };
      } },
      ...
      gen: function (R) {
        ...
        return { type: "pair", prompt: "Rotate $" + pt(x, y) + "$ by $" + name + "$ about the origin.", answer: [im[0], im[1]], near: near,
          ...
          why: "$" + rule + "$, so $" + pt(x, y) + " \\to " + P(im) + "$. " +
            shown([-8, 8], [-8, 8], [{ seg: [[0, 0], [x, y]], c: "soft", dash: "3 4" }, { seg: [[0, 0], im], c: "soft", dash: "3 4" }, { pt: [x, y], name: "P" }, { pt: im, name: "P'", c: "blue" }, { centre: [0, 0], say: "" }]) };
      } },
    ...
    { id: "geo-rshape", title: "Rotating shapes", lesson: 5,
      gen: function (R) {
        ...
        var c = R.chance(0.7) ? [0, 0] : [1, 1], n = R.pick([1, 2, 3]), t = turn(sh, n, c), origin = !c[0] && !c[1];
        var name = n === 1 ? "$90^\\circ$ counterclockwise" : n === 2 ? "$180^\\circ$" : "$90^\\circ$ clockwise";
        var rule = n === 1 ? "(x, y) \\to (-y, x)" : n === 2 ? "(x, y) \\to (-x, -y)" : "(x, y) \\to (y, -x)";
        var a0 = Math.atan2(sh[1][1] - c[1], sh[1][0] - c[0]) * 180 / Math.PI;
        var arc = n === 3 ? { turn: [c, 1.6, a0, a0 - 90], cw: true } : { turn: [c, 1.6, a0, a0 + n * 90] };
        return { type: "plot", prompt: "Draw the image of $\\triangle ABC$ after a rotation of " + name + " about " + (origin ? "the origin" : "the centre $" + P(c) + "$") + ".",
          show: [pre(sh), { centre: c, say: "", at: "sw" }], target: t, x: [-6, 6], y: [-6, 6], u: 26,
          hints: [origin ? "Use the rule for this turn: $" + rule + "$." : "Measure each corner from the centre $" + P(c) + "$, turn that path, then count it out from the centre.",
                  "$A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$."],
          why: (origin ? "$" + rule + "$ gives " : "Turning each corner about $" + P(c) + "$ gives ") + "$A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$. " +
            shown([-6, 6], [-6, 6], [pre(sh), img(t), arc, { centre: c, say: "" }]) };
      } },
    { id: "geo-refpoint", title: "Reflecting points", lesson: 6,
      gen: function (R) {
        ...
        return { type: "pair", prompt: "Reflect $" + pt(x, y) + "$ over " + ml.say + ".", answer: [im[0], im[1]], near: near,
          ...
          why: "$" + pt(x, y) + " \\to " + P(im) + "$: the same distance from the mirror, on the other side. " +
            shown([-9, 9], [-9, 9], [{ mirror: over, say: "" }, { seg: [[x, y], im], c: "orange", dash: "4 4" }, { pt: [x, y], name: "P" }, { pt: im, name: "P'", c: "blue" }]) };
      } },
    ...
    { id: "geo-refshape", title: "Reflecting shapes", lesson: 6,
      gen: function (R) {
        ...
        var ml = L.mirrorLine(over);
        var how = over === "x" ? "Over the $x$-axis, $x$ stays and $y$ changes sign." : over === "y" ? "Over the $y$-axis, $y$ stays and $x$ changes sign."
          : over === "yx" ? "Over $y = x$, the two coordinates of each corner swap." : over === "y-x" ? "Over $y = -x$, swap the coordinates and change both signs."
          : over.x != null ? "Count each corner's distance across to the line $x = " + over.x + "$, then go the same distance past it. The height stays."
          : "Count each corner's distance up or down to the line $y = " + over.y + "$, then go the same distance past it. The $x$ stays.";
        return { type: "plot", prompt: "Draw the reflection of $\\triangle ABC$ over " + ml.say + ".",
          show: [{ mirror: over }, pre(sh)], target: t, x: [-7, 7], y: [-7, 7], u: 22,
          hints: [how, "$A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$."],
          why: "Each corner lands the same distance from " + ml.say + " as it started, on the other side: $A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$. " +
            shown([-7, 7], [-7, 7], [{ mirror: over, say: "" }, pre(sh), img(t), { seg: [sh[0], t[0]], c: "orange", dash: "4 4" }]) };
      } },
      ...
      gen: function (R) {
        ...
        return { type: "pair", prompt: "Dilate $" + pt(x, y) + "$ by a scale factor of $" + kt + "$ about " + (origin ? "the origin" : "the centre $" + P(c) + "$") + ".",
          ...
          why: "$" + P(im) + "$ is on the line from the centre through $P$, $" + kt + "$ times as far from the centre. " +
            shown([-9, 9], [-9, 9], [{ seg: [c, Math.abs(k) >= 1 ? im : [x, y]], c: "soft", dash: "3 4" }, { pt: [x, y], name: "P" }, { pt: im, name: "P'", c: "blue" }, { centre: c, say: "" }]) };
      } },
    ...
    { id: "geo-dshape", title: "Dilating shapes", lesson: 7,
      gen: function (R) {
        var k = R.pick([2, 2, 3, 0.5]);
        var c = k === 3 ? [R.int(-5, -4), R.int(-5, -4)] : [R.int(-4, 0), R.int(-4, 0)];
        var base = R.pick([[[1, 0], [2, 0], [1, 1]], [[1, 1], [3, 1], [1, 2]], [[0, 1], [2, 1], [2, 2]], [[1, 0], [3, 1], [1, 2]]]);
        // Shrinking starts from a figure twice the size, so the image lands on the grid.
        var sh = shift(k === 0.5 ? scale(base, 2) : base, c[0], c[1]), t = scale(sh, k, c), origin = !c[0] && !c[1];
        var kt = k === 0.5 ? "\\frac{1}{2}" : String(k);
        function words(dx, dy) { return [dx ? across(dx) : "", dy ? upDown(dy) : ""].filter(Boolean).join(" and "); }
        var d0 = [sh[0][0] - c[0], sh[0][1] - c[1]];
        return { type: "plot", prompt: "Draw the image of $\\triangle ABC$ under a dilation with scale factor $" + kt + "$ about " + (origin ? "the origin" : "the centre $" + P(c) + "$") + ".",
          show: [pre(sh), { centre: c, say: "", at: "sw" }], target: t, x: [-7, 7], y: [-7, 7], u: 22,
          hints: [origin ? "About the origin, multiply both coordinates of each corner by $" + kt + "$."
                         : "Measure each corner from the centre $" + P(c) + "$, multiply that by $" + kt + "$, and count it out from the centre.",
                  "$A" + P(sh[0]) + "$ is " + words(d0[0], d0[1]) + " from the centre, so $A'$ is " + words(d0[0] * k, d0[1] * k) + " from it: $" + P(t[0]) + "$.",
                  "$A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$."],
          why: "Each corner ends up $" + kt + "$ times as far from the centre, on the same line: $A'" + P(t[0]) + "$, $B'" + P(t[1]) + "$, $C'" + P(t[2]) + "$. " +
            shown([-7, 7], [-7, 7], [{ seg: [c, k > 1 ? t[1] : sh[1]], c: "soft", dash: "3 4" }, { seg: [c, k > 1 ? t[2] : sh[2]], c: "soft", dash: "3 4" }, pre(sh), img(t), { centre: c, say: "" }]) };
      } },
  ...
  /* A problem from a skill, the same one every time, for a lesson's mixed
     review. */
  function mixed(id, i) {
    var sk = SKILLS.filter(function (x) { return x.id === id; })[0];
    var st = sk.gen(L.rng("geo1-review:" + id + ":" + i), i);
    st.skill = sk.title;
    return st;
  }
  ...
  L.unit("geo", 1, {
    ...
    lessons: [
      /* ============================================================== 1 */
      {
        ...
        steps: [
          ...
          recap([
            { m: "A", say: "**Point**: an exact spot, with no size. Named with a capital letter." },
            { m: "\\overline{AB}", say: "**Segment**: the straight piece between two points. It stops at both ends, so it has a length." },
            { m: "\\overrightarrow{AB}", say: "**Ray**: starts at $A$ and goes on forever through $B$. The first letter is always the start." },
            { m: "\\overleftrightarrow{AB}", say: "**Line**: goes on forever both ways. No ends, no length." },
            { say: "**Collinear**: on the same straight line. **Plane**: a flat surface that goes on forever." }],
            FS([piece("point", "A", "", { u: 22 }), piece("seg", "A", "B", { u: 22 }), piece("ray", "A", "B", { u: 22 }), piece("line", "A", "B", { u: 22 })]))
        ]
      },
      /* ============================================================== 2 */
      {
        ...
        steps: [
          ...
          recap([
            { m: "\\angle ABC", say: "**Angle**: two rays from one point, its **vertex**. Name it with the vertex in the middle: $\\angle ABC$, or $\\angle B$ when nothing else meets there." },
            { m: "\\parallel", say: "**Parallel**: in the same plane and never meet." },
            { m: "\\perp", say: "**Perpendicular**: meet in a right angle, $90^\\circ$ — the little square." },
            { say: "**A precise definition** fits every example and nothing else. A **circle** is all the points a fixed distance (the **radius**) from one point (the **centre**)." }],
            FS([angleFig("A", "B", "C", { u: 26 }), linesFig("par", { u: 24 }), linesFig("perp", { u: 24 })]))
        ]
      },
      /* ============================================================== 3 */
      {
        ...
        steps: [
          ...
          { type: "explain", kicker: "In your own words", skill: "Identifying transformations",
            prompt: "Explain the difference between a **rigid** transformation and a **dilation**. Two or three sentences is plenty.",
            model: "A rigid transformation (a translation, rotation or reflection) only moves the figure: every length and every angle stays the same, so the image is an exact copy. A dilation resizes it: every length is multiplied by the scale factor, though the angles, and so the shape, stay the same." },
          recap([
            { m: "A \\to A'", say: "A **transformation** moves every point by one rule. You start with the **pre-image** and end with the **image**; each image point gets a prime." },
            { say: "**Translation** slides. **Rotation** turns about a centre. **Reflection** flips over a line. **Dilation** resizes from a centre." },
            { say: "The first three are **rigid**: same size, same shape. A dilation changes the size." },
            { say: "Which one? Size changed → dilation. Mirror image → reflection. Turned → rotation. Just slid → translation." }])
        ]
      },
      /* ============================================================== 4 */
      {
        title: "Translations", v: 2,
        blurb: "Find the rule by dragging a point, then use it: points, whole figures drawn yourself, and working backwards.",
        mins: 16,
        steps: [
          explore({ kind: "translate", by: [3, -2] }, { start: [-2, 3],
            prompt: "Here is a translation — but not its rule. **Drag** point $P$ around the grid (or click it and use the arrow keys) and watch where its image $P'$ lands. Try at least four places.",
            then: "Wherever $P$ goes, $P'$ is **3 right and 2 down** from it. So every $x$ goes up by 3, and every $y$ goes down by 2." }),
          { type: "choice", prompt: "Look at your table. Which rule does this translation follow?", skill: "Translating points",
            options: [{ t: "$(x, y) \\to (x + 3, y - 2)$" },
                      { t: "$(x, y) \\to (x + 3, y + 2)$", fb: "Look at the $y$ numbers in your table: each new one is 2 **less**." },
                      { t: "$(x, y) \\to (x - 2, y + 3)$", fb: "That swaps them round. The across move goes with $x$, the up-and-down move with $y$." },
                      { t: "$(x, y) \\to (3x, -2y)$", fb: "A translation adds and takes away — it never multiplies." }],
            answer: 0,
            hints: ["Pick one row of your table. How much bigger is the new $x$ than the old one? And the new $y$?"],
            why: "Every row: new $x$ = old $x + 3$, and new $y$ = old $y - 2$." },
          ...
          draw({ prompt: "Now a whole figure — drawn by you. Draw the image of $\\triangle ABC$ under $(x, y) \\to (x - 4, y + 2)$: click where $A'$ goes, then $B'$, then $C'$.",
            skill: "Translating shapes", show: [pre(TRI)], target: shift(TRI, -4, 2), x: [-7, 6], y: [-1, 7],
            hints: ["One corner at a time. $A(1, 1)$ moves 4 left and 2 up, to $(-3, 3)$. Click there.", "$B(4, 1) \\to (0, 3)$ and $C(1, 3) \\to (-3, 5)$."],
            why: "Every corner moves 4 left and 2 up: $A'(-3, 3)$, $B'(0, 3)$, $C'(-3, 5)$. " +
              shown([-7, 6], [-1, 7], TRI.map(function (p) { return { arrow: [p, [p[0] - 4, p[1] + 2]], dash: true }; }).concat([pre(TRI), img(shift(TRI, -4, 2))]), "Triangle ABC and its image, 4 left and 2 up.") }),
          { type: "pair", kicker: "A step on", prompt: "A translation takes $(-3, 4)$ to $(1, 1)$. Where does the **same** translation take $(5, 2)$?", skill: "Determining translations",
            answer: [9, -1],
            near: [{ v: [1, 1], fb: "That's where $(-3, 4)$ went. Find the move first, then do the same move to $(5, 2)$." },
                   { v: [1, 5], fb: "That's the move done backwards. From $(-3, 4)$ to $(1, 1)$ is 4 right and 3 **down**." }],
            hints: ["First find the move. From $(-3, 4)$ to $(1, 1)$: how far across, and how far up or down?", "It's 4 right and 3 down: $(x, y) \\to (x + 4, y - 3)$.", "Now do that to $(5, 2)$: $(5 + 4,\\; 2 - 3)$."],
            why: "The translation is $(x, y) \\to (x + 4, y - 3)$, so $(5, 2) \\to (9, -1)$." },
          ...
          recap([
            { m: "(x, y) \\to (x + a, y + b)", say: "A translation adds the same amounts to every point: $a$ across, $b$ up." },
            { say: "Right and up **add**. Left and down **subtract**." },
            { m: "a = x' - x,\\; b = y' - y", say: "To find a translation from a point and its image: new minus old." },
            { say: "It keeps every length and angle, and the figure faces the same way. Only its position changes." }],
            shown([-5, 5], [-4, 4], [{ steps: [[-3, -2], [2, 1]] }, { pt: [-3, -2], name: "A", at: "nw" }, { pt: [2, 1], name: "A'", c: "blue" }], "A point moved 5 right and 3 up."))
        ]
      },
      /* ============================================================== 5 */
      {
        ...
        steps: [
          ...
          explore({ kind: "rotate", turns: 1, c: [0, 0] }, { start: [4, 1],
            prompt: "This is a $90^\\circ$ counterclockwise turn about the origin. **Drag** point $P$ to four different places and watch the numbers of $P$ and $P'$.",
            then: "Look down the table: the two numbers **swap places**, and the new first number has the **opposite sign**." }),
          { type: "choice", prompt: "Which rule matches your table?", skill: "Rotating points",
            options: [{ t: "$(x, y) \\to (-y, x)$" },
                      { t: "$(x, y) \\to (y, -x)$", fb: "That's the clockwise turn. Check the first row: $(4, 1)$ went to $(-1, 4)$." },
                      { t: "$(x, y) \\to (-x, -y)$", fb: "That's a half turn — both signs change and nothing swaps." },
                      { t: "$(x, y) \\to (y, x)$", fb: "They do swap — but one of them also changes sign." }],
            answer: 0,
            hints: ["Take the first row: $(4, 1) \\to (-1, 4)$. Which rule does that?"],
            why: "$(4, 1) \\to (-1, 4)$: swap to $(1, 4)$, then change the sign of the first number. The next card shows why." },
          ...
          draw({ prompt: "Draw the image of $\\triangle ABC$ after a $90^\\circ$ **clockwise** rotation about the origin.", skill: "Rotating shapes",
            show: [pre(TRI), { centre: [0, 0], say: "" }], target: turn(TRI, -1), x: [-6, 6], y: [-6, 6],
            hints: ["A quarter turn clockwise is $(x, y) \\to (y, -x)$: swap, then change the sign of the second number.", "$A(1, 1) \\to (1, -1)$, $B(4, 1) \\to (1, -4)$, $C(1, 3) \\to (3, -1)$."],
            why: "$(x, y) \\to (y, -x)$ sends $(1, 1)$, $(4, 1)$, $(1, 3)$ to $(1, -1)$, $(1, -4)$, $(3, -1)$. " +
              shown([-6, 6], [-6, 6], [pre(TRI), img(turn(TRI, -1)), { turn: [[0, 0], 2.2, 60, -30], cw: true }, { centre: [0, 0], say: "" }], "Triangle ABC and its image a quarter turn clockwise.") }),
          { type: "learn", kicker: "A step on",
            ...
            gate: true },
          draw({ prompt: "Draw the image of $\\triangle ABC$ after a $180^\\circ$ rotation about the centre $(1, 1)$.", skill: "Rotating shapes",
            show: [pre([[2, 1], [4, 1], [2, 2]]), { centre: [1, 1], at: "nw" }], target: turn([[2, 1], [4, 1], [2, 2]], 2, [1, 1]), x: [-4, 6], y: [-3, 5],
            hints: ["In a half turn, each corner goes straight through the centre to the other side, the same distance away.",
                    "$A$ is 1 right of the centre, so $A'$ is 1 left of it: $(0, 1)$. $B$ is 3 right, so $B'$ is 3 left: $(-2, 1)$. $C$ is 1 right and 1 up, so $C'$ is 1 left and 1 down: $(0, 0)$."],
            why: "Each corner went through the centre $(1, 1)$ to the other side: $A'(0, 1)$, $B'(-2, 1)$, $C'(0, 0)$. " +
              shown([-4, 6], [-3, 5], [pre([[2, 1], [4, 1], [2, 2]]), img(turn([[2, 1], [4, 1], [2, 2]], 2, [1, 1])), { seg: [[2, 2], [0, 0]], c: "soft", dash: "3 4" }, { centre: [1, 1], say: "" }], "The triangle and its half-turn image through the centre (1, 1).") }),
          recap([
            { m: "(x, y) \\to (-y, x)", say: "$90^\\circ$ counterclockwise about the origin: swap, then change the sign of the first number." },
            { m: "(x, y) \\to (-x, -y)", say: "$180^\\circ$: change both signs." },
            { m: "(x, y) \\to (y, -x)", say: "$90^\\circ$ clockwise ($-90^\\circ$): swap, then change the sign of the second number." },
            { say: "Counterclockwise unless it says otherwise. About another centre, measure from the centre. Forgotten a rule? Turn the path from the centre to the point." }],
            shown([-5, 5], [-5, 5], [{ pt: [4, 1], name: "P" }, { pt: [-1, 4], name: "90°", c: "blue", at: "nw" }, { pt: [-4, -1], name: "180°", c: "blue", at: "sw" }, { pt: [1, -4], name: "−90°", c: "blue", at: "se" }, { centre: [0, 0], say: "" }], "P and its images after 90, 180 and minus 90 degree turns."))
        ]
      },
      /* ============================================================== 6 */
      {
        ...
        steps: [
          ...
          { type: "pair", prompt: "Reflect $(-3, 7)$ over the $y$-axis.", skill: "Reflecting points",
            ...
            why: "$(-3, 7) \\to (3, 7)$." },
          explore({ kind: "reflect", over: "yx" }, { start: [4, 1],
            prompt: "The purple line is $y = x$. **Drag** point $P$ and watch its reflection $P'$. Try four places — one of them **on** the line.",
            then: "The two numbers **swap**. And a point on the line stays where it is: it's its own image." }),
          { type: "choice", prompt: "Which rule matches your table?", skill: "Reflecting points",
            options: [{ t: "$(x, y) \\to (y, x)$" },
                      { t: "$(x, y) \\to (-y, -x)$", fb: "That's over the other diagonal, $y = -x$. Here nothing changed sign." },
                      { t: "$(x, y) \\to (-x, y)$", fb: "That's over the $y$-axis." },
                      { t: "$(x, y) \\to (x, -y)$", fb: "That's over the $x$-axis." }],
            answer: 0,
            hints: ["Take the first row: $(4, 1) \\to (1, 4)$."],
            why: "Every row of your table swaps the two numbers. The next card shows why." },
          ...
          draw({ prompt: "Draw the reflection of $\\triangle ABC$ over the line $y = x$.", skill: "Reflecting shapes",
            show: [{ mirror: "yx" }, pre([[3, 0], [6, 0], [5, 2]])], target: flip([[3, 0], [6, 0], [5, 2]], "yx"), x: [-1, 7], y: [-1, 7],
            hints: ["Over $y = x$, the two coordinates of each corner swap.", "$A(3, 0) \\to (0, 3)$, $B(6, 0) \\to (0, 6)$, $C(5, 2) \\to (2, 5)$."],
            why: "Each corner swaps its coordinates: $A'(0, 3)$, $B'(0, 6)$, $C'(2, 5)$. " +
              shown([-1, 7], [-1, 7], [{ mirror: "yx", say: "" }, pre([[3, 0], [6, 0], [5, 2]]), img(flip([[3, 0], [6, 0], [5, 2]], "yx")), { seg: [[5, 2], [2, 5]], c: "orange", dash: "4 4" }], "The triangle and its mirror image across y = x.") }),
          draw({ kicker: "A step on", prompt: "Draw the reflection of $\\triangle ABC$ over the line $x = -1$.", skill: "Reflecting shapes",
            show: [{ mirror: { x: -1 } }, pre(TRI)], target: flip(TRI, { x: -1 }), x: [-7, 6], y: [-2, 5],
            hints: ["Count each corner's distance to the line $x = -1$, then go the same distance past it. The height stays the same.",
                    "$A(1, 1)$ is 2 right of the line, so $A'$ is 2 left of it: $(-3, 1)$. $B(4, 1) \\to (-6, 1)$ and $C(1, 3) \\to (-3, 3)$."],
            why: "Each corner is the same distance from $x = -1$ as its image, on the other side: $A'(-3, 1)$, $B'(-6, 1)$, $C'(-3, 3)$. " +
              shown([-7, 6], [-2, 5], [{ mirror: { x: -1 }, say: "" }, pre(TRI), img(flip(TRI, { x: -1 })), { seg: [[4, 1], [-6, 1]], c: "orange", dash: "4 4" }], "The triangle and its reflection over x = −1.") }),
          ...
          recap([
            { m: "(x, y) \\to (x, -y)", say: "Over the $x$-axis: $x$ stays, $y$ changes sign." },
            { m: "(x, y) \\to (-x, y)", say: "Over the $y$-axis: $y$ stays, $x$ changes sign." },
            { m: "(x, y) \\to (y, x)", say: "Over $y = x$: swap. Over $y = -x$: swap and change both signs, $(-y, -x)$." },
            { say: "Over any other line: count to it, then the same again past it. The mirror is halfway between a point and its image, at a right angle to the segment joining them. A point on the mirror doesn't move." }],
            shown([-5, 5], [-4, 4], [{ mirror: "y" }, { seg: [[-4, 1], [4, 1]], c: "orange", dash: "4 4" }, { pt: [-4, 1], name: "Q", at: "nw" }, { pt: [4, 1], name: "Q'", c: "blue" }], "A point and its reflection across the y-axis."))
        ]
      },
      /* ============================================================== 7 */
      {
        ...
        steps: [
          ...
          explore({ kind: "dilate", k: 2, c: [0, 0] }, { start: [2, 1],
            prompt: "This is a dilation with scale factor 2 about the origin. **Drag** point $P$ to four places and watch $P'$.",
            then: "$P'$ is always on the line from the centre through $P$, **twice as far out** — so both of its numbers are doubled." }),
          { type: "choice", prompt: "Which rule matches your table?", skill: "Dilating points",
            options: [{ t: "$(x, y) \\to (2x, 2y)$" },
                      { t: "$(x, y) \\to (x + 2, y + 2)$", fb: "Check the first row: $(2, 1)$ went to $(4, 2)$. The $y$ went from 1 to 2, not 3 — it doubled." },
                      { t: "$(x, y) \\to (2x, y)$", fb: "Both numbers double, not just $x$." },
                      { t: "$(x, y) \\to (\\frac{x}{2}, \\frac{y}{2})$", fb: "That would shrink it, halfway to the centre. These images are further out." }],
            answer: 0,
            hints: ["Take the first row: $(2, 1) \\to (4, 2)$."],
            why: "Every row doubles both numbers: $(x, y) \\to (2x, 2y)$." },
          ...
          draw({ prompt: "Draw the image of $\\triangle ABC$ under a dilation with scale factor 2 about the origin.", skill: "Dilating shapes",
            show: [pre([[1, 1], [3, 1], [1, 2]]), { centre: [0, 0], say: "" }], target: scale([[1, 1], [3, 1], [1, 2]], 2), x: [-1, 7], y: [-1, 5],
            hints: ["About the origin, multiply both coordinates of each corner by 2.", "$A(1, 1) \\to (2, 2)$, $B(3, 1) \\to (6, 2)$, $C(1, 2) \\to (2, 4)$."],
            why: "Every coordinate doubles: $A'(2, 2)$, $B'(6, 2)$, $C'(2, 4)$ — each twice as far from the origin, on the same line. " +
              shown([-1, 7], [-1, 5], [{ ray: [[0, 0], [6, 2]], c: "soft", dash: "3 4", bare: true }, { ray: [[0, 0], [2, 4]], c: "soft", dash: "3 4", bare: true }, pre([[1, 1], [3, 1], [1, 2]]), img(scale([[1, 1], [3, 1], [1, 2]], 2)), { centre: [0, 0], say: "" }], "The triangle and its image twice as big.") }),
          draw({ kicker: "A step on", prompt: "Draw the image of $\\triangle ABC$ under a dilation with scale factor $\\frac{1}{2}$ about the centre $(-3, -1)$.", skill: "Dilating shapes",
            show: [pre([[1, 3], [5, 3], [1, 5]]), { centre: [-3, -1], at: "nw" }], target: scale([[1, 3], [5, 3], [1, 5]], 0.5, [-3, -1]), x: [-4, 6], y: [-2, 6],
            hints: ["Measure from the centre, not the origin. $A(1, 3)$ is 4 right and 4 up from $(-3, -1)$.", "Half of that is 2 right and 2 up, so $A'(-1, 1)$. In the same way, $B'(1, 1)$ and $C'(-1, 2)$."],
            why: "Each corner comes halfway in towards $(-3, -1)$: $A'(-1, 1)$, $B'(1, 1)$, $C'(-1, 2)$. " +
              shown([-4, 6], [-2, 6], [{ seg: [[-3, -1], [5, 3]], c: "soft", dash: "3 4" }, { seg: [[-3, -1], [1, 5]], c: "soft", dash: "3 4" }, pre([[1, 3], [5, 3], [1, 5]]), img(scale([[1, 3], [5, 3], [1, 5]], 0.5, [-3, -1])), { centre: [-3, -1], say: "" }], "The triangle and its half-size image, pulled in towards the centre.") }),
          ...
          recap([
            { m: "(x, y) \\to (kx, ky)", say: "About the origin: multiply both coordinates by the scale factor $k$." },
            { say: "About another centre: find how far the point is from the centre, multiply that by $k$, and count it out from the centre." },
            { m: "k = \\frac{\\text{image}}{\\text{pre-image}}", say: "More than 1 makes it bigger; between 0 and 1, smaller." },
            { say: "Lines through each point and its image all meet at the centre. Angles never change — only lengths — so the shape stays the same." }],
            shown([-1, 7], [-1, 4], [{ ray: [[0, 0], [2, 1]], c: "soft", dash: "3 4" }, { pt: [2, 1], name: "P", at: "nw" }, { pt: [6, 3], name: "P'", c: "blue", at: "nw" }, { centre: [0, 0], say: "" }], "P and its image three times as far from the origin."))
        ]
      },
      /* ============================================================== 8 */
      {
        ...
        steps: [
          ...
          { type: "explain", kicker: "In your own words", skill: "Identifying transformations",
            prompt: "How can you tell whether a figure was **rotated** or **reflected**? Explain it the way you'd tell a friend.",
            model: "Imagine the figure cut out of paper. If turning the paper is enough to make it land on the image, it was rotated. If you have to flip the paper over, it was reflected: the image is a mirror image, so its corners go round in the opposite order." },
          recap([
            { say: "**1.** Size changed? **Dilation**. **2.** Mirror image? **Reflection**. **3.** Turned? **Rotation**. Otherwise: **translation**." },
            { say: "Rigid moves keep lengths, angles and area, so the image is **congruent** to the pre-image. A dilation keeps only the angles: the image is **similar** — same shape, different size." },
            { say: "A reflection is the only rigid move that turns a figure over." }])
        ]
      },
      /* ============================================================== 9 */
      {
        title: "Unit review", v: 1,
        blurb: "Every rule on one card, then a mixed set where you have to choose the move yourself.",
        mins: 15,
        steps: [
          { type: "learn", kicker: "Your toolkit",
            prompt: "Everything in this unit, on one card.",
            art: FS([
              grid([-3, 5], [-2, 4], [{ steps: [[-2, -1], [3, 2]] }, { pt: [-2, -1], name: "P", at: "nw" }, { pt: [3, 2], name: "P'", c: "blue" }], { u: 20,
                cap: "**Translation** $(x, y) \\to (x + a, y + b)$. Find it: new minus old.", alt: "A point translated 5 right and 3 up." }),
              grid([-4, 4], [-4, 4], [{ pt: [3, 1], name: "P" }, { pt: [-1, 3], name: "90°", c: "blue", at: "nw" }, { pt: [-3, -1], name: "180°", c: "blue", at: "sw" }, { pt: [1, -3], name: "−90°", c: "blue", at: "se" }, { centre: [0, 0], say: "" }], { u: 20,
                cap: "**Rotation** about the origin: $90^\\circ$ $(-y, x)$; $180^\\circ$ $(-x, -y)$; $-90^\\circ$ $(y, -x)$.", alt: "A point and its images after quarter, half and three-quarter turns." }),
              grid([-4, 4], [-4, 4], [{ mirror: "yx", say: "" }, { mirror: "y", say: "" }, { pt: [3, 1], name: "P" }, { pt: [1, 3], name: "y=x", c: "blue", at: "nw" }, { pt: [-3, 1], name: "y-axis", c: "blue", at: "nw" }], { u: 20,
                cap: "**Reflection**: $x$-axis $(x, -y)$; $y$-axis $(-x, y)$; $y = x$ $(y, x)$; $y = -x$ $(-y, -x)$.", alt: "A point and its reflections over y = x and the y-axis." }),
              grid([-1, 7], [-1, 4], [{ ray: [[0, 0], [6, 3]], c: "soft", dash: "3 4", bare: true }, { pt: [2, 1], name: "P", at: "nw" }, { pt: [6, 3], name: "P'", c: "blue", at: "nw" }, { centre: [0, 0], say: "" }], { u: 20,
                cap: "**Dilation** about the origin $(kx, ky)$; about $C$, multiply the distance from $C$. $k$ = image ÷ pre-image.", alt: "A point and its image three times as far from the origin." })]),
            after: "Which move? Size changed → dilation. Mirror image → reflection. Turned → rotation. Just slid → translation. <br><br>The next problems are mixed up, the way a test is: decide which rule you need before you start." }
        ].concat(["geo-name", "geo-tpoint", "geo-tdet", "geo-tshape", "geo-tchal", "geo-rpoint", "geo-rdet", "geo-rshape",
                  "geo-refpoint", "geo-refdet", "geo-refshape", "geo-dpoint", "geo-dscale", "geo-dcenter", "geo-dshape"].map(function (id, i) { return mixed(id, i); }))
      }
    ],
    ...
    quizzes: [
      ...
      { title: "Quiz 2", after: 5, blurb: "Translations and rotations: points, rules, and figures you draw.",
        skills: ["geo-tpoint", "geo-tdet", "geo-tshape", "geo-rpoint", "geo-rshape"], per: 2 },
      { title: "Quiz 3", after: 7, blurb: "Reflections and dilations: points, figures you draw, and scale factors.",
        skills: ["geo-refpoint", "geo-refshape", "geo-dpoint", "geo-dscale", "geo-dshape"], per: 2 }
    ],
    skills: SKILLS
  });
})();
